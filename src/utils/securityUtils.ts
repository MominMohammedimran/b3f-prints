 ...
 * Validates if current session meets security requirements
 * @returns Promise<boolean> indicating if session is secure
 */
export const checkSessionSecurity = async (): Promise<boolean> => {
  ...
  // If any security check fails, show a warning
  if (!isSecureContext) {
    toast.warning('Warning: Not running in a secure context. Your connection may not be private.');
  }
  
  if (!isSessionStorageAvailable) {
    toast.warning('Warning: Session storage is not available. Some features may not work properly.');
  }
  
  if (!areCookiesEnabled) {
    toast.warning('Warning: Cookies are disabled. Please enable cookies for authentication to work properly.');
  }
  
  // Return true if all security checks pass
  return isSecureContext && isSessionStorageAvailable && areCookiesEnabled;
};
